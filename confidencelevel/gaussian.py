...
def test_unconstrained():
    ...
    confint = GaussianUnconstrainedInterval(measured=measured, sigma=sigma, cl=1.0-alpha)
    ll_int, ul_int = confint.confidence_interval()
    P.axvline(ll_int, ls='--', c='r', label='calculated CI boundaries')
    P.axvline(ul_int, ls='--', c='r')

    P.plot(x, confint.likelihood_ratio(x), '--m', label='likelihood ratio')
    # P.plot(x, confint.critical_value(x), '--m', label='crit. LR')
    P.axhline(confint.critical_value(x), ls=':', c='b', label='crit. LR')
